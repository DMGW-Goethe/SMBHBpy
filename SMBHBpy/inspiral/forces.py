...
class DynamicalFriction(DissipativeForce):
    ...
    def F(self, sp, r, v, e, opt):
        ...
        and with velocity v
        The self.ln_Lambda is the Coulomb logarithm, for which different authors use different values. Set to -1 so that Lambda = b_max/r_infl()
        Parameters:
            sp (SystemProp) : The object describing the properties of the inspiralling system
        ...
        if ln_Lambda < 0.:
            ln_Lambda = np.log(sp.b_max/sp.r_infl())
