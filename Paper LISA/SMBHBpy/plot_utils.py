...
from SMBHBpy import halo, inspiral, waveform, constants as c

def plotEvolution(sp, ev, ax_a=None, ax_e=None, label="", ax_ae=None, ax_n=None, color=None, linestyle=None):
    """
    Plots the evolution of the system in the natural units that are used throughout the code.
    The evolution can be plotted as semimajor axis / time, eccentricity / time, eccentricity / semimajor axis,
    ...
    """
    if not ax_a is None:
        l, = ax_a.loglog(ev.t/c.year_to_pc, ev.a/(sp.r_isco_1()+sp.r_isco_2()), label=label, color=color, linestyle=linestyle)
        color = l.get_c()
    if not ax_e is None:
        l, = ax_e.loglog(ev.t/c.year_to_pc, ev.e, color=color, label=label, linestyle=linestyle)
        color = l.get_c()
    if not ax_ae is None:
        l, = ax_ae.plot(ev.a/(sp.r_isco_1()+sp.r_isco_2()), ev.e, color=color, label=label, linestyle=linestyle)
        color = l.get_c()
    if not ax_n is None:
